...
import { Download, FileText, CheckCircle, AlertCircle, Upload, Eye, CheckCircle2, RefreshCw } from "lucide-react"
...
export default function DocumentationStep({ companyData, onComplete, bankDetails, onNavigateToStep }: DocumentationProps) {
  ...
  // Balance payment receipt upload state
  const [isBalancePaymentUploading, setIsBalancePaymentUploading] = useState(false)
  const [balancePaymentUploadProgress, setBalancePaymentUploadProgress] = useState(0)
  ...
  const checkAnyUploadInProgress = () => {
    return Object.values(uploadingStates).some(uploading => uploading)
  }
  ...
  const [documents, setDocuments] = useState({
    ...
    form19: companyData.form19 || null,
  ...
  const [signedDocuments, setSignedDocuments] = useState<Record<string, any>>({
    form1: null,
    form19: null,
    aoa: null,
  ...
  useEffect(() => {
    ...
    if (companyData.customerDocuments) {
      setSignedDocuments({
        ...
        form19: companyData.customerDocuments.form19 || null,
        ...
        ...(companyData.step3SignedAdditionalDoc && Object.keys(companyData.step3SignedAdditionalDoc).reduce((acc: Record<string, any>, key, index) => {
  ...
  // Fetch available packages directly from MySQL database and listen for real-time updates
  useEffect(() => {
    ...
    const handlePackagesUpdate = async () => {
      console.log('📦 Customer Step 3 - Packages updated event received, reloading from API...')
      await loadPackages()
    }

    // Listen for packages-updated event
    window.addEventListener('packages-updated', handlePackagesUpdate as EventListener)
    loadPackages()

    return () => {
      window.removeEventListener('packages-updated', handlePackagesUpdate as EventListener)
    }
  ...
  const handleForm18Upload = async (index: number, file: File) => {
    ...
    try {
      ...
      // Clear any existing interval for this upload key
      if (progressIntervals.current[uploadKey]) {
        clearInterval(progressIntervals.current[uploadKey]);
      }
      ...
      const progressInterval = setInterval(() => {
        setUploadProgress(prev => {
          ...
          delete progressIntervals.current[uploadKey];
          return prev;
        });
      }, 200);

      // Store the interval reference
      progressIntervals.current[uploadKey] = progressInterval;
  ...
  const saveBalancePaymentReceipt = async (file: File) => {
    ...
    try {
      ...
      // Set upload state
      setIsBalancePaymentUploading(true);
      setBalancePaymentUploadProgress(0);

      // Simulate progress updates
      progressInterval = setInterval(() => {
        setBalancePaymentUploadProgress(prev => {
          if (prev < 90) {
            return prev + 10
          }
          if (progressInterval) {
      ...
      if (uploadResult.success && uploadResult.file) {
        ...
        // Update the payment receipt in the component state with uploaded file info
        setPaymentReceipt(balancePaymentReceiptData)
        ...
        // Complete progress and save to MySQL database
        setBalancePaymentUploadProgress(100);
      ...
      // Clear interval and reset upload state
      if (progressInterval) {
        clearInterval(progressInterval);
      }
      setTimeout(() => {
        setIsBalancePaymentUploading(false);
        setBalancePaymentUploadProgress(0);
      }, 500);
    }
  }
  ...
  const saveCustomerDocumentToDatabase = async (documentType: string, document: any) => {
    try {
      ...
      const customerDocuments: Record<string, any> = {
        form1: documentType === 'form1' ? document : currentSignedDocuments.form1,
        form19: documentType === 'form19' ? document : currentSignedDocuments.form19,
  ...
  const saveForm18DocumentToDatabase = async (index: number, document: any) => {
    try {
      ...
      const customerDocuments: Record<string, any> = {
        ...
        form19: currentSignedDocuments.form19,
  ...
  const additionalDocumentsUploaded = companyData.additionalDocuments && companyData.additionalDocuments.length > 0
    ? companyData.additionalDocuments
      .filter((doc: any) => doc.signRequired !== false) // Only check documents that require signature
      .every((doc: any, index: number) => {
        // Find the actual index in the original array
        const actualIndex = companyData.additionalDocuments.findIndex((d: any) => d === doc);
        return !!signedDocuments[`additional_${actualIndex}`];
      })
    : true
  ...
  const step3AdditionalDocumentsUploaded = companyData.step3AdditionalDoc && companyData.step3AdditionalDoc.length > 0
    ? companyData.step3AdditionalDoc
      .filter((doc: any) => doc.signRequired !== false) // Only check documents that require signature
      .every((doc: any, index: number) => {
        // Find the actual index in the original array
        const actualIndex = companyData.step3AdditionalDoc.findIndex((d: any) => d === doc);
        return !!signedDocuments[`step3_additional_${actualIndex}`];
      })
    : true // Step 3 additional documents are now optional

  const allSignedDocumentsUploaded =
    signedDocuments.form1 &&
    signedDocuments.form19 &&
    signedDocuments.aoa &&
  ...
  console.log('📝 Customer Step 3 - allSignedDocumentsUploaded check:', {
    ...
    requiredAdditionalDocumentsCount: companyData.additionalDocuments?.filter((doc: any) => doc.signRequired !== false).length || 0,
    step3AdditionalDocumentsCount: companyData.step3AdditionalDoc?.length || 0,
    step3RequiredDocumentsCount: companyData.step3AdditionalDoc?.filter((doc: any) => doc.signRequired !== false).length || 0,
    additionalDocumentsUploaded,
    step3AdditionalDocumentsUploaded,
    step3AdditionalDocumentsDetails: companyData.step3AdditionalDoc?.map((doc: any, index: number) => ({
      title: doc.title,
      signRequired: doc.signRequired,
      uploaded: !!signedDocuments[`step3_additional_${index}`]
    })),
    step3RequiredDocumentsDetails: companyData.step3AdditionalDoc?.filter((doc: any) => doc.signRequired !== false).map((doc: any) => ({
      title: doc.title,
      signRequired: doc.signRequired
    })),
        ...
        if (companyIndex !== -1) {
          ...
          const customerDocuments: Record<string, any> = {
            form1: signedDocuments.form1 && typeof signedDocuments.form1 === 'object'
              ...
              : null,
            form19: signedDocuments.form19 && typeof signedDocuments.form19 === 'object'
              ? {
                ...signedDocuments.form19,
                signedByCustomer: true,
                submittedAt: new Date().toISOString(),
    ...
    <div className="space-y-6">
      ...
      <Card className="w-full">
              ...
              >
                <div className="grid grid-cols-1 gap-4">
                  ...
                  <DocumentDownloadCard
                    ...
                    document={documents.form19}
                    onDownload={() => handleDownload(documents.form19)}
                  />
                  ...
                  {directors.map((director: any, idx: number) => (
                    <DocumentDownloadCard
                      key={idx}
                      title={`FORM 18 - ${(director && typeof director === 'object' && (
                        director.name ||
                        director.fullName ||
                        ((director.firstName || director.lastName) ? `${director.firstName || ''} ${director.lastName || ''}`.trim() : '')
                      )) || `Director ${idx + 1}`
                        }`}
                      description={`Consent to act as director (${(director && typeof director === 'object' && (
                        director.name ||
                        director.fullName ||
                        ((director.firstName || director.lastName) ? `${director.firstName || ''} ${director.lastName || ''}`.trim() : '')
                      )) || `Director ${idx + 1}`
                  ...
                  {companyData.step3AdditionalDoc && companyData.step3AdditionalDoc.length > 0 && (
                    <>
                      <div className="col-span-full mt-4 mb-2">
                        ...
                        <p className="text-sm text-gray-600">Additional documents provided by the administrator for step 3 (optional)</p>
                      </div>
              ...
              >
                <div className="grid grid-cols-1 gap-4">
                  <DocumentUploadCard
                    ...
                    isUploading={uploadingStates["form1"]}
                    uploadProgress={uploadProgress["form1"]}
                    isAnyUploadInProgress={isAnyUploadInProgress}
                  />

                  <DocumentUploadCard
                    title="FORM 19"
                    description="Upload signed Consent and Certificate of Secretary"
                    document={signedDocuments.form19}
                    onUpload={(file) => handleFileUpload("form19", file)}
                    isUploading={uploadingStates["form19"]}
                    uploadProgress={uploadProgress["form19"]}
                    isAnyUploadInProgress={isAnyUploadInProgress}
                  />
                  ...
                  <DocumentUploadCard
                    ...
                    isUploading={uploadingStates["aoa"]}
                    uploadProgress={uploadProgress["aoa"]}
                    isAnyUploadInProgress={isAnyUploadInProgress}
                  />
                  ...
                  {directors.map((director: any, idx: number) => (
                    <DocumentUploadCard
                      ...
                      description={`Upload signed director consent form (${(director && typeof director === 'object' && (
                        director.name ||
                        director.fullName ||
                        ((director.firstName || director.lastName) ? `${director.firstName || ''} ${director.lastName || ''}`.trim() : '')
                      )) || `Director ${idx + 1}`
                      ...
                      isUploading={uploadingStates[`form18-${idx}`]}
                      uploadProgress={uploadProgress[`form18-${idx}`]}
                      isAnyUploadInProgress={isAnyUploadInProgress}
                    />
                  ))}

                  {/* Address Proof Document Upload - Only show if business address number is not provided */}
                  {(!companyData.businessAddressNumber || companyData.businessAddressNumber.trim() === "") && (
                    <DocumentUploadCard
                      ...
                      isUploading={uploadingStates["addressProof"]}
                  ...
                  {companyData.additionalDocuments && companyData.additionalDocuments.length > 0 && (
                    <>
                      ...
                      {companyData.additionalDocuments
                        .filter((doc: any) => doc.signRequired !== false) // Only show documents that require signature
                        .map((doc: any, index: number) => {
                          // Find the actual index in the original array
                          const actualIndex = companyData.additionalDocuments.findIndex((d: any) => d === doc);
                          return (
                            <DocumentUploadCard
                              key={`additional-upload-${actualIndex}`}
                              title={`Signed ${doc.title}`}
                              description={`Upload signed version of ${doc.title} (signature required)`}
                              document={signedDocuments[`additional_${actualIndex}`] || null}
                              onUpload={(file) => handleFileUpload(`additional_${actualIndex}`, file)}
                              isUploading={uploadingStates[`additional_${actualIndex}`]}
                              uploadProgress={uploadProgress[`additional_${actualIndex}`]}
                              isAnyUploadInProgress={isAnyUploadInProgress}
                            />
                          );
                        })}
                    ...
                    <>
                      ...
                      {companyData.step3AdditionalDoc
                        ...
                        .map((doc: any, index: number) => {
                          ...
                          return (
                            <DocumentUploadCard
                              key={`step3-additional-upload-${actualIndex}`}
                              title={`Signed ${doc.title}`}
                              description={`Upload signed version of ${doc.title} (signature required)`}
                              document={signedDocuments[`step3_additional_${actualIndex}`] || null}
                              onUpload={(file) => handleFileUpload(`step3_additional_${actualIndex}`, file)}
                              isUploading={uploadingStates[`step3_additional_${actualIndex}`]}
                              uploadProgress={uploadProgress[`step3_additional_${actualIndex}`]}
                              isAnyUploadInProgress={isAnyUploadInProgress}
                            />
                          );
                        })}
        ...
        <CardFooter className="flex justify-end">
          {activeTab === "upload" && (() => {
            const selectedPackage = getPackageById(availablePackages, companyData.selectedPackage)
            const isAdvanceBalance = isAdvanceBalancePackage(selectedPackage)

            // Hide the button if it's an advance+balance package and payment section is shown
            if (isAdvanceBalance && showPaymentSection) {
              return null
            }
                ...
                >
                  ...
                  {(() => {
                    ...
                    console.log('🔍 Customer Step 3 - Button text debugging:', {
                      ...
                      isAdvanceBalance: isAdvanceBalance,
        ...
        return (
          <Card ref={balancePaymentRef} className="mt-6">
            ...
            <CardContent className="space-y-6">
              {(() => {
                try {
                  const packageBalanceAmount = selectedPackage?.balanceAmount || 0
                  const additional: any = (companyData as any).additionalFees || {}
                  const directorsTotal = Number(additional?.directors?.total || 0)
                  const shareholdersTotal = Number(additional?.shareholders?.total || 0)
                  const additionalFeesTotal = Number(additional?.total || 0)
                  const adminAdjustmentAmount = Number(additional?.adminAdjustment?.amount || 0)
                  const adminAdjustment = additional?.adminAdjustment
                  const totalBalancePayment = packageBalanceAmount + additionalFeesTotal + adminAdjustmentAmount

                  return (
                    <div className="p-4 rounded-md bg-blue-50 border border-blue-200 text-blue-900">
                      <div className="font-semibold mb-3 text-lg">Balance Payment Calculation</div>

                      <div className="flex justify-between items-center py-2 border-b border-blue-200">
                        <span className="text-sm font-medium">Package Balance Amount:</span>
                        <span className="text-sm font-semibold">Rs. {packageBalanceAmount.toLocaleString()}</span>
                      </div>

                      <div className="flex justify-between items-center py-2 border-b border-blue-200">
                        <span className="text-sm font-medium">Directors Charges:</span>
                        <span className="text-sm font-semibold">Rs. {directorsTotal.toLocaleString()}</span>
                      </div>

                      <div className="flex justify-between items-center py-2 border-b border-blue-200">
                        <span className="text-sm font-medium">Shareholders Charges:</span>
                        <span className="text-sm font-semibold">Rs. {shareholdersTotal.toLocaleString()}</span>
                      </div>
                      ...
                      {adminAdjustment && (
                        <div className="flex justify-between items-center py-2 border-b border-blue-200">
                          <span className="text-sm font-medium">
                            {adminAdjustment.amount >= 0 ? 'Additional Charges' : 'Discount'}
                            ({adminAdjustment.reason}):
                          </span>
                          <span className={`text-sm font-semibold ${adminAdjustment.amount >= 0 ? 'text-red-600' : 'text-green-600'}`}>
                            {adminAdjustment.amount >= 0 ? '+' : ''}Rs. {Math.abs(adminAdjustment.amount).toLocaleString()}
                          </span>
                        </div>
                      )}

                      <div className="flex justify-between items-center py-2 mt-2">
                        <span className="text-base font-bold">Total Balance Payment:</span>
                        <span className="text-base font-bold text-blue-700">Rs. {totalBalancePayment.toLocaleString()}</span>
                      </div>
                    </div>
                  )
                } catch {
              ...
              {isBalancePaymentRejected && (
                <Alert className="bg-red-50 border-red-200">
                  ...
                  <AlertDescription className="text-red-700">
                    ...
                    <br />
                    <strong>Please ensure your new receipt is clear and shows the correct payment amount of Rs. {(() => {
                      const packageBalanceAmount = selectedPackage?.balanceAmount || 0
                      const additional: any = (companyData as any).additionalFees || {}
                      const additionalFeesTotal = Number(additional?.total || 0)
                      const adminAdjustmentAmount = Number(additional?.adminAdjustment?.amount || 0)
              ...
              <Alert className="bg-blue-50 border-blue-200">
                ...
                <AlertDescription className="text-blue-700">
                  Please transfer <b>Rs. {(() => {
                    ...
                    const totalBalancePayment = packageBalanceAmount + additionalFeesTotal + adminAdjustmentAmount
                    return totalBalancePayment.toLocaleString()
                  })()}</b> as the balance payment for the <b>{selectedPackage?.name}</b> package.
                </AlertDescription>
              </Alert>
                ...
                <div className="space-y-4">
                  ...
                  <BankTransferDetails
                    ...
                    onFileUpload={(file: File) => {
                      // Immediately save; state will be updated after upload completes
                      saveBalancePaymentReceipt(file)
                    }}
                    uploadedFile={paymentReceipt}
                    existingReceipt={companyData.balancePaymentReceipt}
                    isUploading={isBalancePaymentUploading}
                    uploadProgress={balancePaymentUploadProgress}
                  />
                </div>
              )}
            ...
            <CardFooter className="flex justify-end">
              <Button
                onClick={onSubmit}
                disabled={isSubmitting || !paymentReceipt || isBalancePaymentUploading}
                className="w-full sm:w-auto"
              >
                {isSubmitting ? "Processing..." : isBalancePaymentUploading ? "Uploading Receipt..." : isBalancePaymentRejected ? "Resubmit Balance Payment" : "Complete Balance Payment"}
              </Button>
            </CardFooter>
          </Card>
        )
      ...
      <CardContent className="p-4">
        ...
        <div className="mt-2 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
          <div className="flex items-center min-w-0">
            ...
            <span className="text-xs sm:text-sm truncate">{document.name}</span>
          </div>
          <div className="flex gap-2 w-full sm:w-auto">
            <Dialog>
              <DialogTrigger asChild>
                <Button variant="ghost" size="sm" className="h-10 px-3 flex-1 sm:flex-none">
...
function DocumentUploadCard({
  ...
  const fileInputId = `${title.replace(/\s+/g, "-").toLowerCase()}-${Math.random().toString(36).substr(2, 9)}`
  const fileInputRef = useRef<HTMLInputElement>(null)
  const replaceFileInputRef = useRef<HTMLInputElement>(null) // Add separate ref for replace input
  ...
  const uploading = isUploading !== undefined ? isUploading : isUploadingState

  // Disable the card if this specific upload is in progress OR if any other upload is in progress
  const isCardDisabled = uploading || (globalUploadInProgress && !uploading)

  return (
    <Card className={`w-full ${document ? "border-green-200 bg-green-50/30" : "border-dashed"}`}>
      <CardContent className="p-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
          <div>
            <h3 className="font-medium text-sm sm:text-base">{title}</h3>
          ...
          {document && <CheckCircle className="h-5 w-5 text-green-500 hidden sm:block" />}
        </div>
        ...
        {document ? (
              ...
              >
                {uploading ? (
                  <>
                    ...
                    <span className="text-xs sm:text-sm">Replacing...</span>
                  </>
                ) : (
                  <>
          ...
          <div className="mt-2">
            <input
              ...
              disabled={isCardDisabled}
            />
            <label htmlFor={fileInputId} className={`w-full ${isCardDisabled ? "opacity-50 cursor-not-allowed" : ""}`}>
              <Button variant="outline" asChild disabled={isCardDisabled} className="h-10 px-3 w-full">
