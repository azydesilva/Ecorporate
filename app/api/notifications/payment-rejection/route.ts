...
export async function POST(request: NextRequest) {
    try {
        ...
        const { registrationId, companyName, name: nameFromBody, packageName, rejectionReason } = body;
        ...
        console.log('🔍 Payment rejection - Looking up registration:', { registrationId, companyName });
        ...
        // Send email notification for payment rejection
        if (resolvedEmail && resolvedName && resolvedCompanyName) {
            try {
                console.log('📧 Sending payment rejection email to:', resolvedEmail);
                await sendPaymentRejectionEmail({
                    to: resolvedEmail,
                    name: resolvedName,
                    companyName: resolvedCompanyName,
                    packageName: packageName || undefined,
                    rejectionReason: rejectionReason || 'Payment details require review. Please check your payment receipt and ensure all information is correct.'
                });
                console.log('✅ Payment rejection email sent successfully to:', resolvedEmail);
            } catch (emailError) {
        ...
        return NextResponse.json({
            success: true,
            message: 'Payment rejection processed successfully',
            data: {
                id: registrationId,
        ...
        return NextResponse.json(
            {
                error: 'Failed to process payment rejection',
