...
import { sendSecretaryRecordsSubmittedEmail } from '@/lib/email-service';
...
export async function POST(request: NextRequest) {
    try {
        ...
        // Send the secretary records submitted email to the resolved recipient (user's registered email)
        const result = await sendSecretaryRecordsSubmittedEmail({
            to: resolvedEmail,
            name: finalName,
        ...
        return NextResponse.json({
            success: true,
            message: 'Secretary records submitted email sent successfully',
            data: result
        });
        ...
        return NextResponse.json(
            {
                error: 'Failed to send secretary records submitted email',
