...
import { sendNameApprovalEmail } from '@/lib/email-service';
...
export async function POST(request: NextRequest) {
    try {
        ...
        const { registrationId, companyName, name: nameFromBody } = body;
        ...
        const result = await sendNameApprovalEmail({
            to: resolvedEmail,
            name: finalName,
            companyName: finalCompanyName
        });

        return NextResponse.json({ success: true, message: 'Name approval email sent successfully', data: result });
    } catch (error) {
        console.error('Error sending name approval email:', error);
        return NextResponse.json(
            { error: 'Failed to send name approval email', details: error instanceof Error ? error.message : 'Unknown error' },
            { status: 500 }
        );
    }
}