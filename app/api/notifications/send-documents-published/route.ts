...
import { sendDocumentsPublishedEmail } from '@/lib/email-service';
...
export async function POST(request: NextRequest) {
    try {
        ...
        // Send the documents published email to the resolved recipient (user's registered email)
        const result = await sendDocumentsPublishedEmail({
            to: resolvedEmail,
            name: finalName,
        ...
        return NextResponse.json({
            success: true,
            message: 'Documents published email sent successfully',
            data: result
        });
        ...
        return NextResponse.json(
            {
                error: 'Failed to send documents published email',
