...
import { sendSignedSecretaryRecordsEmail } from '@/lib/email-service';
...
export async function POST(request: NextRequest) {
    try {
        ...
        // Send the signed secretary records email to the resolved recipient (user's registered email)
        const result = await sendSignedSecretaryRecordsEmail({
            to: resolvedEmail,
            name: finalName,
        ...
        return NextResponse.json({
            success: true,
            message: 'Signed secretary records email sent successfully',
            data: result
        });
        ...
        return NextResponse.json(
            {
                error: 'Failed to send signed secretary records email',
