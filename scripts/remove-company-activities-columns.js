// Migration script to remove Company Activities columns from registrations table
const mysql = require('mysql2/promise');
require('dotenv').config();

async function removeCompanyActivitiesColumns() {
    let connection;

    try {
        console.log('🚀 Starting migration to remove Company Activities columns...');
        ...
        // Show final table structure (Company Activities related columns)
        console.log('\n📋 Verifying Company Activities columns are removed:');
        ...
        if (stillExists.length > 0) {
            ...
            console.log('✅ All Company Activities columns successfully removed');
        }

        console.log('\n✅ Company Activities removal migration completed successfully!');
...
if (require.main === module) {
    removeCompanyActivitiesColumns()
        .then(() => {
            console.log('🎉 Company Activities removal migration completed successfully');
            process.exit(0);
        })
        .catch((error) => {
            console.error('💥 Migration failed:', error);
...
module.exports = removeCompanyActivitiesColumns;